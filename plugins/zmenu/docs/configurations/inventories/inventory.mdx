...
<Tabs>
    ...
    <TabItem value="name-list" label="Name with List">
        Each line was joined together to create a single title.
    ...
    <TabItem value="title" label="Title">
        ```yaml
        title: "&6&lMy Server Menu"
        ```
    </TabItem>
</Tabs>

**Aliases:** `name` and `title` are interchangeable. You can use either one to define the inventory title.

**Per-language translations:**

Use `translated-name` (or `translatedName`) to supply a different title for each client locale. Falls back to `name` when no matching locale is found.
...
When you use `type: ANVIL`, the inventory becomes a **text input**: the player can type text in the anvil rename field. Each time the text changes, zMenu runs the requirements defined under `rename-requirements`, giving you live validation of what the player is typing.

The anvil has 3 slots: the two input slots (`0` and `1`) and the output/result slot (`2`).

```yaml
name: "&8Type something"
type: ANVIL
...
items:
  ...
  result:
    ...
    item:
      ...
      name: "&aYou typed: &f%new_text%"
```

**Placeholders available inside `rename-requirements`:**

| Placeholder  | Description                                                              |
|--------------|--------------------------------------------------------------------------|
| `%type%`     | The kind of change: `ADDED`, `REMOVED`, `CLEARED`, `REPLACED` or `EQUAL` |
| `%old_text%` | The text before the change                                               |
| `%new_text%` | The current text in the rename field                                     |
| `%char%`     | The character that was added or removed (only for `ADDED` / `REMOVED`)   |

:::info
The result item placed in slot `2` is sent directly to the player and is updated on every keystroke, which lets you build a live preview of what the player is typing.
:::
...
### clear-inventory-type

Choice what type of inventory clear is used to clear the player's inventory.

```yaml
clear-inventory-type: DEFAULT
```

**Options:**
- `DEFAULT` - Clears the inventory using the default method (clears the player's inventory and saves the contents to restore later)
- `PACKET_EVENT` - Clears the inventory using PacketEvents (sends packets to the player to clear their inventory without actually modifying it, so items remain in their inventory but are not visible)
...
### click-limiter-enabled

Enable a click limiter to prevent players from clicking too fast.

```yaml
click-limiter-enabled: true
```
...
Useful for profile menus or any inventory opened for a specific player.
...
<Tabs>
    <TabItem value="fill-item" label="fill-item">
        ```yaml
        fill-item:
    ...
    <TabItem value="fillItem" label="fillItem">
        ```yaml
        fillItem:
          material: BLACK_STAINED_GLASS_PANE
          name: "&8"
...
**Aliases:** `fill-item` and `fillItem` are interchangeable.
...
Requirements that must be met to open this inventory. Similar to `view-requirement`, but checked before opening the inventory, allowing you to prevent the inventory from opening at all if conditions aren't met.

<Tabs>
    <TabItem value="open-requirement" label="open-requirement">
        ```yaml
        open-requirement:
          requirements:
            - type: permission
    ...
    <TabItem value="open_requirement" label="open_requirement">
        ```yaml
        open_requirement:
          requirements:
            - type: permission
...
**Aliases:** `open_requirement` and `open-requirement` are interchangeable.

---

### inventory-replacement

Automatically redirect Bedrock players to a native Bedrock form when they try to open this inventory. Java players will see the regular inventory as usual.

:::warning Requirements
This feature requires **Geyser** or **Floodgate** installed on your server.
:::
...
| Option | Type | Default | Description |
|--------|------|---------|-------------|
| `name` | String | - | The Bedrock form file name (without `.yml`) |
| `plugin` | String | `zMenu` | The plugin owning the Bedrock form |
| `pages` | List | empty | Only redirect on these pages (empty = all pages) |

See [Bedrock Forms](../bedrock) for complete documentation on creating Bedrock forms.
...
<Tabs>
    <TabItem value="open-with-item" label="open-with-item">
        ```yaml
        open-with-item:
          item:
            material: COMPASS
    ...
    <TabItem value="openWithItem" label="openWithItem">
        ```yaml
        openWithItem:
          item:
            material: COMPASS
...
**Aliases:** `open-with-item` and `openWithItem` are interchangeable.

**Actions types:**
- `RIGHT_CLICK` - Right click in air + on blocks
- `LEFT_CLICK` - Left click in air + on blocks
- `RIGHT_CLICK_AIR` - Right click in air only
- `LEFT_CLICK_AIR` - Left click in air only
- `RIGHT_CLICK_BLOCK` - Right click on a block only
- `LEFT_CLICK_BLOCK` - Left click on a block only
